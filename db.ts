...
export async function initializeDatabase() {
  ...
  // Migration for setups that still have the legacy call_transcripts table.
  await pool.query(`ALTER TABLE call_reviews ADD COLUMN IF NOT EXISTS call_transcripts JSONB`);
  await pool.query(`
    DO $$
    BEGIN
      IF EXISTS (
        ...
        SET call_transcripts = jsonb_build_object(
          'transcriptText',
          ct.transcript_text,
          'transcriptJson',
          ct.transcript_json
        ...
        WHERE ct.call_id = cr.call_id
          AND cr.call_transcripts IS NULL;
...
export async function saveCallAnalysis(input: {
  ...
  try {
    ...
    await client.query(
      `INSERT INTO call_reviews (call_id, average_score, call_transcripts, summary, feedback_text, facts_json, scores_json, created_at)
      ...
      [
        ...
        JSON.stringify({
          ...
          transcriptJson: input.transcriptJson ?? null,
        }),
        input.summary || null,
